...
const body_finalization = @import("connection_body_finalization.zig");
const body_types = @import("connection_body_types.zig");
const connection_chunked_body = @import("connection_chunked_body.zig");
const request_head = @import("../http1/request_head.zig");
const worker_response_staging = @import("worker_response_staging.zig");
...
fn prepareApplicationBody(
    ...
    const finite_output = if (comptime @hasDecl(@TypeOf(storage.*), "finiteOutput"))
        ...
        @TypeOf(request.finite_output) == @import("../application_finite_output.zig").Plan)
        request.finite_output
    else
        @import("../application_finite_output.zig").Plan.contiguous;
