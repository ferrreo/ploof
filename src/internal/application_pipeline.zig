const application_compile = @import("application_compile.zig");
const application_csrf = @import("application_csrf.zig");
const application_pipeline_serialize = @import("application_pipeline_serialize.zig");
const application_pipeline_support = @import("application_pipeline_support.zig");
const application_pipeline_html = @import("application_pipeline_html.zig");
const application_pipeline_selected_body = @import("application_pipeline_selected_body.zig");
const application_response_output = @import("application_response_output.zig");
const application_stream_pipeline = @import("application_stream_pipeline.zig");
...
pub fn Pipeline(
    ...
    return struct {
        ...
        pub fn runSelectedBodyResponse(
            ...
            if (comptime @import("../html_response.zig").is(Payload)) {
            ...
            inline for (middleware, 0..) |item, index| {
                if (@hasDecl(@TypeOf(item), "body")) {
                    if (invokeBody(
                        ...
                        context,
                        &states[index],
                        @import("../application_context.zig").Bodyless{},
                        mapped_error,
                    )) |value| return value;
                }
            }
