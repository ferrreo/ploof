...
const input_materialize = @import("application_input_materialize.zig");
...
pub fn plan(comptime handler: anytype) Plan {
    ...
    if (comptime @hasDecl(Handler, "ploof_input_endpoint")) {
        return @import("application_input.zig").plan(Handler);
    }
...
pub fn materializeSelectedCsrf(
    ...
    workspace: []u8,
    json_hash_key: [16]u8,
    csrf_state: ?*@import("csrf_request.zig").State,
