...
const asset_response = @import("internal/asset_response.zig");
...
test "If-None-Match selects negotiated ETag and emits bodyless 304" {
    ...
    try std.testing.expectEqualStrings("", prepared.body);
    try std.testing.expectEqual(@import("response.zig").Status.not_modified, prepared.status);
...
test "Range request is ignored and the complete representation is returned" {
    ...
    try std.testing.expectEqual(@import("response.zig").Status.ok, prepared.status);
...
test "identity can be rejected without advertising range support" {
    ...
    try std.testing.expectEqual(@import("response.zig").Status.not_acceptable, prepared.status);
