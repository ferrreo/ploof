...
const multipart_file_sink = @import("multipart_file_sink.zig");
const response = @import("response.zig");
const route = @import("route.zig");
const startup = @import("startup.zig");
const catalog = @import("internal/application_upload_catalog.zig");
...
test "registry keeps typed runtime drivers in catalog declaration order" {
    ...
    try std.testing.expect(@TypeOf(anonymous_driver) ==
        *@import("internal/upload_sink_driver.zig").Runtime(Anonymous));
    try std.testing.expect(@TypeOf(named_driver) ==
        *@import("internal/upload_sink_driver.zig").Runtime(NamedDurable));
...
test "startup configuration report views fixed application FileSink catalog" {
    ...
    try std.testing.expectEqual(
        @sizeOf(@import("internal/runtime/worker_upload_route_metrics.zig").Cell),
